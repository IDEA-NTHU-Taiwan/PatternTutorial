...
def apply_by_multiprocessing(df, func, **kwargs):
    ...
    if coln == 1:
        ...
        series_0, series_1 = series.apply(
            lambda x: x[0]), series.apply(lambda x: x[1])
        return series_0, series_1
    elif coln == 3:
        series_0, series_1, series_2 = series.apply(lambda x: x[0]), series.apply(
            lambda x: x[1]), series.apply(lambda x: x[2])
